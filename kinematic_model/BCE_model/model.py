...
class KinematicModel(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(x.size(0)):
            enc_x = self.cnn_lstm(x[i]) # [batch, feat_channel]
            enc_features.append(enc_x)
        x = torch.stack(enc_features) # [gestures, batch, feat_channel]
        x = x.permute(1, 2, 0) # [batch, feat_channel, gestures]         
        x = self.pool(x)
        x = x.view(x.size(0), -1)
        x = self.relu1(x)
        x = self.norm1(x)
...
class CNN_LSTM(nn.Module):
    def __init__(self, input_channels, output_channels):
        ...
        self.conv1 = nn.Conv1d(input_channels, 4, kernel_size=21, stride=4, padding=10)
                
        self.lstm1 = nn.LSTM(4, 4, num_layers=1, batch_first=True, bidirectional=True)
        ...
        self.dropout = nn.Dropout(p=0.5)        
        self.fc = nn.Linear(8, output_channels)
